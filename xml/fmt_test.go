...
const xml1 = `<root><this><is>a</is><test /><message><!-- with comment --><org><cn>Some org-or-other</cn><ph>Wouldnt you like to know</ph></org><contact><fn>Pat</fn><ln>California</ln></contact></message></this></root>`
...
func TestFormatXML_t5(t *testing.T) {
	person := struct {
		...
		Height:    178,
		Married:   false,
		Address:   struct{ City, State string }{City: "NewYork", State: "Washendon"},
		Comment:   `This is a example`,
	}

	if data, err := xml.Marshal(&person); err != nil {
		t.Errorf("xml.Marshal %v", err)
	} else {
		t.Logf("Person: %s", FormatXML(string(data), "", "	"))
	}
